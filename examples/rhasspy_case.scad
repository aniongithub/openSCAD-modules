...
LID_HEIGHT_FRAC = CASE_HEIGHT * 0.3;
...
module lid()
{
    difference()
    {
        translate([0, 0, LID_HEIGHT_FRAC / 2])
        difference()
        {
            union()
            {
                difference()
                {
                    roundCornersCube(CASE_LENGTH + CASE_THICKNESS + CASE_THICKNESS, CASE_WIDTH + CASE_THICKNESS + CASE_THICKNESS, LID_HEIGHT_FRAC, CASE_CORNER_RADIUS);
                    translate([0, 0, CASE_THICKNESS])
                        roundCornersCube(CASE_LENGTH + CASE_THICKNESS, CASE_WIDTH + CASE_THICKNESS, LID_HEIGHT_FRAC + CASE_THICKNESS, CASE_CORNER_RADIUS);

                }
            }
        }
        for (x_i = [0:LID_HOLES_X - 1])
        {
            x = (CASE_LENGTH - CASE_THICKNESS - 5) / (LID_HOLES_X - 1);
            for (y_i = [0:LID_HOLES_Y - 1])
            {
                y = (CASE_WIDTH - CASE_THICKNESS - 5) / (LID_HOLES_Y - 1);
                translate([-CASE_LENGTH / 2 + 5 + x * x_i , -CASE_WIDTH / 2 + 5 + y * y_i, -CASE_THICKNESS / 2])
                cylinder(CASE_THICKNESS + 5, d = LID_HOLE_DIA, $fn = 36);
            }
        }
    }
}
...
module case()
{
    ...
    translate([PI_OFFSET_X, PI_OFFSET_Y, CASE_THICKNESS])
    {
